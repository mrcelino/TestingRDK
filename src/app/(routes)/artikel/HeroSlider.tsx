...
export default function HeroSlider() {
  ...
  return (
    <div className="relative w-full h-[36vh] lg:h-screen">
      ...
      >
        {articles.length > 0 ? (
          articles.filter((article) => article.category === "Big_Agenda_Ramadhan").map((article) => (
            <SwiperSlide key={article.id}>
              <div className="relative w-full h-full">
                ...
                {/* Judul Artikel */}
                <div className="absolute lg:bottom-16 bottom-5 text-center font-semibold italic text-white p-4 lg:w-[65%] block lg:mx-[14rem] z-20 w-full lg:text-3xl text-lg">
                  {article.title} 
                </div>
