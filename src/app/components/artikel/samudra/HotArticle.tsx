...
<<<<<<< HEAD:src/app/components/artikel/samudra/HotArticle.tsx
...
>>>>>>> 7e356a584f05ce2bcead4b15596178d4027896c3:src/app/(routes)/artikel/samudra/HotArticle.tsx
...
//   const [loading, setLoading] = useState(true);
  const hotArticles = articles.filter(
    (slide) =>
      slide.category === "Samudera" && slide.like >= 50 && slide.like <= 100
  );

  // Jika Hot Articles kosong, menggunakan semua artikel dengan kategori "Samudera"
      ...
      {isLoading ? (
        <div className="flex space-x-4 py-6">
          {Array.from({ length: 2 }).map((_, index) => (
            <div
              key={index}
              className="bg-gray-200 animate-pulse w-[90%] lg:w-[1000px] lg:h-[500px] rounded-3xl"></div>
          ))}
        </div>
